    ...
    override fun analyze(image: ImageProxy) {
        ...
        if ((image.format == ImageFormat.YUV_420_888 || image.format == ImageFormat.YUV_422_888
            ...
            try {
                ...
                //Meter el Fuel para la solicitud
                val httpAsync = "https://melescaso.com/api/invitado/validar_entrada/${rawResult.text}"
                    .httpGet()
                    .responseString { request, response , result ->
                        when (result){
                            is Result.Failure -> {
                                val ex = result.getException()
                                val resultEx = Gson().fromJson(ex.toString(),ResponseMeLesCaso::class.java)
                                Log.d("CLIENTE HTTP", resultEx.message)
                                listener.onScanned(resultEx.message)
                            }
                            is Result.Success -> {
                                val data = result.get()
                                val resultSucc = Gson().fromJson(data,ResponseMeLesCaso::class.java)
